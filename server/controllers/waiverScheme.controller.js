...
export default {
  ...
  async list(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to retrieve waiver schemes." });
    }
  },
  /**
  ...
  async getById(req, res) {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res
        .status(400)
        .json({ error: "Invalid Waiver Scheme ID format." });
    }
    try {
      const waiverScheme = await WaiverScheme.findOne({ _id: id }).lean();

      if (!waiverScheme) {
        return res.status(404).json({ error: "Waiver Scheme not found." });
      }

      if (!["admin", "staff", "manager"].includes(req.user.type)) {
      ...
      if (
        waiverScheme.required_documents &&
        Array.isArray(waiverScheme.required_documents)
      ) {
        waiverScheme.required_documents.forEach((doc) => {
          if (
            doc &&
            doc.schema_id &&
            doc.schema_id !== "aadhaar_card" &&
            doc.schema_id !== "pan_card"
          ) {
      ...
      if (waiverScheme.fields && Array.isArray(waiverScheme.fields)) {
        waiverScheme.fields.forEach((field) => {
          if (
            field.auto_fill_sources &&
            Array.isArray(field.auto_fill_sources)
          ) {
      ...
      const aadhaarDef = await GovDocumentDefinitionModel.findOne({
        schema_id: "aadhaar_card",
      }).lean();
      const panDef = await GovDocumentDefinitionModel.findOne({
        schema_id: "pan_card",
      }).lean();
      ...
      if (aadhaarDef) {
        ...
        console.warn(
          "Aadhaar card definition (schema_id: 'aadhaar_card') not found in GovDocumentDefinitionModel."
        );
      }
      ...
      if (panDef) {
        ...
        console.warn(
          "PAN card definition (schema_id: 'pan_card') not found in GovDocumentDefinitionModel."
        );
      }
  ...
  async update(req, res) {
    ...
    try {
      ...
      const updatedWaiverScheme = await WaiverScheme.findByIdAndUpdate(
        id,
        req.body,
        {
  ...
  async remove(req, res) {
    ...
    try {
      ...
      if (waiverScheme.status === "archived") {
        return res.json({
          message: "Waiver Scheme is already archived.",
          waiverScheme,
        });
      }
      ...
      return res.json({
        message: "Waiver Scheme archived successfully.",
        waiverScheme: archivedScheme,
      });
    } catch (err) {
