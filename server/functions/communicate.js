import {SENDER_EMAIL, SENDER_PASSWORD, SMTP_SERVER, SMTP_PORT} from "../config.js"; // Adjust the path as necessary
...
const FAST2SMS_API_KEY = process.env.FAST2SMS_API_KEY || 'ZxzPcJ8GfksSWaXOvYbhC6EujlKRq47NTQw2mV139IDnLyrF5pSInJLcXB4aqwOmTQ9HvboGdWVri2ey';
...
async function sendOtp(req, dltPrincipalEntityId = null) {
    ...
    if (FAST2SMS_API_KEY === 'YOUR_FAST2SMS_API_KEY' || !FAST2SMS_API_KEY) {
        console.warn("Fast2SMS API Key is not configured. Please set it in environment variables or directly in the code (not recommended for production).");
    ...
    console.log(`Generated OTP for ${mobileNumber} (Request ID: ${uniqueRequestId}): ${generatedOtp}`);
    ...
    try {
        ...
        const isSuccessMessage = Array.isArray(response.data.message) &&
                                 response.data.message.some(msg => typeof msg === 'string' && msg.toLowerCase().includes('sent successfully'));
        
        // Handle cases where 'errors_keys' might be present even with return: true
        const hasErrorKeys = response.data && Array.isArray(response.data.errors_keys) && response.data.errors_keys.length > 0;
        ...
        if (isSuccessReturn && isSuccessMessage && !hasErrorKeys) {
            ...
            if (response.data && response.data.message) {
                apiErrorMessage = Array.isArray(response.data.message) ? response.data.message.join('; ') : String(response.data.message);
            }
            if (hasErrorKeys) { // Prepend error keys if they exist
                apiErrorMessage = `Error Keys: [${response.data.errors_keys.join(', ')}] - ${apiErrorMessage}`;
            }
            ...
            console.error(`Fast2SMS API Error for ${mobileNumber} (Request ID: ${uniqueRequestId}):`, apiErrorMessage, response.data);
        ...
        console.error(`Error sending OTP to ${mobileNumber} (Request ID: ${uniqueRequestId}):`, error.response ? error.response.data : error.message);
...
async function verifyOtp(req) {
    ...
    if (!requestId || !userOtp) {
        throw new Error("Request ID (requestId) and OTP (otp) are required in request body for verification.");
    }

    const storedOtpData = otpStore[requestId];

    if (!storedOtpData) {
        throw new Error("Invalid Request ID or OTP session not found. It might have expired or was never initiated. Please request a new OTP.");
    }

    if (storedOtpData.verified) {
        console.log(`OTP for Request ID ${requestId} (Number: ${storedOtpData.phoneNumber}) was already verified.`);
        return { message: "OTP already verified successfully." };
    }

    if (storedOtpData.attempts >= MAX_VERIFY_ATTEMPTS) {
        delete otpStore[requestId];
        throw new Error(`Maximum verification attempts reached for Request ID ${requestId}. Please request a new OTP.`);
    }

    const currentTime = Date.now();
    if (currentTime > storedOtpData.timestamp + OTP_EXPIRY_DURATION) {
        delete otpStore[requestId];
        throw new Error(`OTP for Request ID ${requestId} has expired. Please request a new OTP.`);
    }
    ...
    if (userOtp === storedOtpData.otp) {
        ...
        console.log(`OTP for Request ID ${requestId} (Number: ${storedOtpData.phoneNumber}) verified successfully on attempt ${storedOtpData.attempts}.`);
        return { message: "OTP verified successfully." };
    } else {
        storedOtpData.attempts += 1;
        console.warn(`Invalid OTP attempt ${storedOtpData.attempts} for Request ID ${requestId} (Number: ${storedOtpData.phoneNumber}). User provided: ${userOtp}, Expected: ${storedOtpData.otp}`);
...
async function testOtpFlow() {
    ...
    try {
        ...
        if (FAST2SMS_API_KEY === 'YOUR_FAST2SMS_API_KEY' || !FAST2SMS_API_KEY) {
            console.error("Please configure FAST2SMS_API_KEY in environment variables or directly in the code before testing.");
            return;
        }
         if (!process.env.OTP_APPLICATION_NAME && APPLICATION_NAME === "Your Service") {
            console.warn("Consider setting OTP_APPLICATION_NAME environment variable for a more specific OTP message.");
        }
        
        const sendResult = await sendOtp(mockSendReq);
        console.log(sendResult.message, "Request ID:", sendResult.requestId, "Generated OTP (for debug):", sendResult.otp_for_debug);

        const otpToVerify = sendResult.otp_for_debug;
        const receivedRequestId = sendResult.requestId;

        if (!otpToVerify || !receivedRequestId) {
            console.error("Could not retrieve OTP or Request ID for verification test.");
            return;
        }
        ...
        console.log(`Attempting to verify OTP ${otpToVerify} for Request ID ${receivedRequestId}...`);
        ...
        console.log(`Attempting to verify with invalid OTP for Request ID ${receivedRequestId}...`);
        await verifyOtp(mockVerifyReqInvalid).catch(err => console.error("Verification error (expected for invalid OTP):", err.message));
        
        // Test already verified OTP
        console.log(`Attempting to re-verify the valid OTP ${otpToVerify} for Request ID ${receivedRequestId}...`);
