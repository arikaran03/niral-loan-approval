...
const LoanFormBuilder = ({
  ...
  const getInitialLoanDataState = useCallback(
    () => ({
      ...
      required_documents: Array.isArray(initialData?.required_documents)
        ? initialData.required_documents.filter(
            doc => doc.name?.toLowerCase() !== 'aadhaar card' && doc.name?.toLowerCase() !== 'pan card'
          ).map((d) => ({ ...d }))
        : [],
  ...
  useEffect(() => {
    const fetchGovDocDefinitions = async () => {
      ...
      try {
        ...
        if (!aadhaarDef) console.warn(`Aadhaar schema definition (schema_id: ${AADHAAR_SCHEMA_ID}) not found.`);
        if (!panDef) console.warn(`PAN schema definition (schema_id: ${PAN_SCHEMA_ID}) not found.`);

        const otherDocs = definitions.filter(
          def => def.schema_id !== AADHAAR_SCHEMA_ID && def.schema_id !== PAN_SCHEMA_ID
        );
        ...
        setDocDefError("Could not load document types. Auto-fill sources and standard document selection might be affected.");
      } finally {
  ...
  const validate = (isPublishingValidation = false) => {
    ...
    if (isPublishingValidation && !loanData.description?.trim()) newErrors.description = "Description is required for publishing.";

    // Financial Details
    if ((isPublishingValidation || loanData.min_amount !== "") && (isNaN(loanData.min_amount) || parseFloat(loanData.min_amount) < 0)) newErrors.min_amount = "Minimum amount must be a non-negative number.";
    if ((isPublishingValidation || loanData.max_amount !== "") && (isNaN(loanData.max_amount) || parseFloat(loanData.max_amount) < 0)) newErrors.max_amount = "Maximum amount must be a non-negative number.";
    ...
    if ((isPublishingValidation || loanData.interest_rate !== "") && (isNaN(loanData.interest_rate) || parseFloat(loanData.interest_rate) < 0)) newErrors.interest_rate = "Interest rate must be a non-negative number.";
    if ((isPublishingValidation || loanData.tenure_months !== "") && (isNaN(loanData.tenure_months) || parseInt(loanData.tenure_months, 10) < 1)) newErrors.tenure_months = "Tenure must be at least 1 month.";
    ...
    // Dates
    if (isPublishingValidation && !loanData.application_start) newErrors.application_start = "Application start date is required for publishing.";
    if (isPublishingValidation && !loanData.application_end) newErrors.application_end = "Application end date is required for publishing.";
    ...
    loanData.fields.forEach((field, index) => {
      ...
      if (!isDerived && !currentFieldId?.trim()) newErrors[`${prefix}.field_id`] = "Field ID is required (or select an auto-fill source).";
      if (currentFieldId && fieldIds.has(currentFieldId.trim())) newErrors[`${prefix}.field_id`] = `Field ID "${currentFieldId}" is already used or derived. IDs must be unique.`;
      ...
      if ((field.type === "select" || field.type === "multiselect") && (!field.options || field.options.length === 0)) newErrors[`${prefix}.options`] = "Options are required for select/multiselect types (comma-separated).";
    });
    ...
    updateLoanDataState((prev) => {
      ...
      if (isMultiSelect) {
        ...
        if (selectedValues.length > 0) {
          ...
          if (derivedKey && (!currentManualId || wasDerived)) fieldToUpdate["field_id"] = derivedKey;
        } else if (oldAutoFillSources.length > 0 && selectedValues.length === 0 && wasDerived) {
  ...
  const handleAddOtherDocumentRequirement = () => {
    ...
    const selectedDocDef = otherDocTypesForSelection.find(def => def._id === selectedOtherDocumentType);
    if (!selectedDocDef || loanData.required_documents.some(doc => doc.name === selectedDocDef.name)) {
        // Prevent adding if not found or already added by name
        console.warn("Document not found or already added:", selectedOtherDocumentType);
        return;
    }

    // Generate description from the definition's fields
    const fieldsDescription = (selectedDocDef.fields || []).map(f => f.label).join(", ");
    const description = fieldsDescription ? `Expected Fields: ${fieldsDescription}` : "General document requirement.";

    const newRequirement = {
      name: selectedDocDef.name, // Use name from definition
      description: description,
      // Optionally, store the schema_id or _id of the GovDocumentDefinition for reference
      // gov_doc_definition_id: selectedDocDef._id 
    };
    updateLoanDataState((prev) => ({
      ...prev,
   ...
   const handleAddCustomField = () => {
    updateLoanDataState((prev) => ({
      ...
      fields: [
        ...prev.fields,
        {
          field_id: "", field_label: "", field_prompt: "", type: "text",
          required: false, min_value: "", max_value: "", options: [], auto_fill_sources: [],
        },
      ],
    }));
  };
  ...
  const renderInput = (name, label, type = "text", options = {}, valueOverride = undefined) => {
    ...
    return (
      <Form.Group as={Col} md={options.md || 6} className="mb-3" key={fieldId} controlId={fieldId}>
        ...
        <Form.Control type={type} name={name} value={value} onChange={handleInputChange}
          checked={type === "checkbox" ? !!value : undefined}
          required={options.required} min={options.min} max={options.max} step={options.step}
          placeholder={options.placeholder} isInvalid={isInvalid} disabled={internalIsSaving} />
  ...
  const renderTextArea = (name, label, options = {}, valueOverride = undefined) => {
    ...
    return (
      <Form.Group as={Col} md={options.md || 12} className="mb-3" key={fieldId} controlId={fieldId}>
        <Form.Label>{label}{options.required ? " *" : ""}</Form.Label>
        <Form.Control as="textarea" name={name} value={value} onChange={handleInputChange}
          rows={options.rows} placeholder={options.placeholder} required={options.required}
          isInvalid={isInvalid} disabled={internalIsSaving} />
        <Form.Control.Feedback type="invalid">{errors[name]}</Form.Control.Feedback>
      </Form.Group>
    );
  };
  ...
  const renderCustomFieldInput = (index, fieldName, label, type = "text", options = {}) => {
    ...
    const isDerived = isIdField && fieldData?.auto_fill_sources?.length > 0 && value === getFieldKeyFromSource(fieldData.auto_fill_sources[0]);
      ...
      return (
        <Col md={options.md || 6} className="mb-3">
          <Form.Group className="h-100 d-flex align-items-end" key={controlId} controlId={controlId}>
            <div> 
              <Form.Check type="switch" name={fieldName} label={label} checked={!!value} 
                onChange={(e) => handleCustomFieldChange(index, e)} isInvalid={isInvalid} 
    ...
    return (
      <Form.Group as={Col} md={options.md || 6} className="mb-3" key={controlId} controlId={controlId}>
        <Form.Label>{label}{options.required && !isDerived ? " *" : ""}</Form.Label>
        <Form.Control type={type} name={fieldName} value={value} 
          onChange={(e) => handleCustomFieldChange(index, e)} readOnly={isDerived} 
          className={isDerived ? "read-only-input" : ""} required={options.required && !isDerived} 
          min={options.min} max={options.max} step={options.step} placeholder={options.placeholder} 
          isInvalid={isInvalid} disabled={internalIsSaving} />
  ...
  const renderCustomFieldTextarea = (index, fieldName, label, options = {}) => {
    ...
    if (fieldName === "options" && Array.isArray(value)) value = value.join(", ");
    value = value ?? ""; 
    const isInvalid = !!errors[fullFieldName];
    return (
      <Form.Group as={Col} md={options.md || 12} className="mb-3" key={controlId} controlId={controlId}>
        <Form.Label>{label}{options.required ? " *" : ""}</Form.Label>
        <Form.Control as="textarea" name={fieldName} value={value} 
          onChange={(e) => handleCustomFieldChange(index, e)} rows={options.rows} 
          placeholder={options.placeholder} required={options.required} isInvalid={isInvalid} 
          disabled={internalIsSaving} />
        <Form.Control.Feedback type="invalid">{errors[fullFieldName]}</Form.Control.Feedback>
      </Form.Group>
    );
  ...
  const renderCustomFieldSelect = (index, fieldName, label, optionsArray = [], fieldOptions = {}) => {
    ...
    return (
      <Form.Group as={Col} md={fieldOptions.md || 6} className="mb-3" key={controlId} controlId={controlId}>
        ...
        <Form.Select name={fieldName} value={value} onChange={(e) => handleCustomFieldChange(index, e)} 
          required={fieldOptions.required} isInvalid={isInvalid} disabled={internalIsSaving}>
  ...
  const renderCustomFieldMultiSelect = (index, fieldName, label, baseOptionsArray = [], fieldOptions = {}) => {
    ...
    let filteredOptions = baseKey ? baseOptionsArray.filter(opt => opt.key === baseKey || selectedSources.includes(opt.value)) : baseOptionsArray;

    return (
      <Form.Group as={Col} md={fieldOptions.md || 12} className="mb-3" key={controlId} controlId={controlId}>
        <Form.Label>{label}{fieldOptions.required ? " *" : ""}</Form.Label>
        <Form.Select multiple name={fieldName} value={selectedSources} 
          onChange={(e) => handleCustomFieldChange(index, e)} required={fieldOptions.required}
          className={`multi-select ${isInvalid ? "is-invalid" : ""}`} style={{ minHeight: "120px" }} 
          disabled={internalIsSaving}>
    ...
    <div className="loan-form-builder-component">
      ...
      <Form ref={formRef} onSubmit={handlePublish} className="loan-form-builder-content needs-validation" noValidate>
        ...
        <Card className="mb-4 shadow-sm">
          ...
          <Card.Body>
            <Row>
              {renderInput("title", "Loan Title", "text", { required: true, md: 12, placeholder: "Enter a clear title" })}
              {renderTextArea("description", "Description (HTML supported)", { rows: 5, md: 12, required: false, placeholder: "Describe the loan..." })}
            </Row>
          </Card.Body>
        </Card>
          ...
          <Card.Body className="bg-light-subtle">
            <Row>
              ...
              <Col md={12} className="mb-3 mt-2">
                <Form.Check type="switch" id="field_collateral_required" name="collateral_required"
                  label="Collateral Required?" checked={!!loanData.collateral_required}
                  onChange={handleInputChange} disabled={internalIsSaving} />
              </Col>
            </Row>
          </Card.Body>
        </Card>
            ...
            <Card.Body>
                ...
                {!docDefLoading && (!aadhaarSchemaDef || !panSchemaDef) && (
                    <Alert variant="warning" size="sm">
                        Aadhaar or PAN card schema definitions could not be loaded. These are mandatory for loan products.
                        Please ensure schemas with ID '<code>{AADHAAR_SCHEMA_ID}</code>' and '<code>{PAN_SCHEMA_ID}</code>' exist.
                    </Alert>
                )}
                ...
                <Accordion defaultActiveKey={['0','1']} alwaysOpen>
                    {aadhaarSchemaDef && (
                        <Accordion.Item eventKey="0">
                            <Accordion.Header><FaIdCard className="me-2"/>Aadhaar Card (Mandatory)</Accordion.Header>
                            <Accordion.Body>
                                <p className="text-muted small">{aadhaarSchemaDef.description}</p>
                                <ListGroup variant="flush">
                                    {(aadhaarSchemaDef.fields || []).map(field => (
                                        <ListGroup.Item key={field.key} className="py-1 px-0 border-0">
                                            <strong>{field.label}:</strong> <span className="text-muted fst-italic">({field.prompt})</span>
                                        </ListGroup.Item>
                                    ))}
                                </ListGroup>
                            </Accordion.Body>
                    ...
                    {panSchemaDef && (
                         <Accordion.Item eventKey="1">
                            <Accordion.Header><FaIdCard className="me-2"/>PAN Card (Mandatory)</Accordion.Header>
                            <Accordion.Body>
                                <p className="text-muted small">{panSchemaDef.description}</p>
                                <ListGroup variant="flush">
          ...
          <Card.Body>
            ...
            {!docDefLoading && otherDocTypesForSelection.length > 0 && (
                ...
                <Col xs={12} sm>
                    ...
                    onChange={(e) => setSelectedOtherDocumentType(e.target.value)}
                    ...
                    {otherDocTypesForSelection.map((docDef) => (
                        <option key={docDef._id} value={docDef._id}> 
                ...
                <Col xs={12} sm="auto">
                    ...
                    disabled={internalIsSaving || !selectedOtherDocumentType || docDefLoading}
                    className="w-100"
                    >
                    Add Document
                    </Button>
             ...
             {errors.required_documents && (
                <Alert variant="danger" size="sm" className="mt-2">{errors.required_documents}</Alert>
            )}
            <ListGroup variant="flush">
              ...
              {loanData.required_documents.map((doc, index) => (
                <ListGroup.Item key={`req-doc-${index}`} className="d-flex justify-content-between align-items-center ps-1 pe-1">
                  <div className="flex-grow-1 me-2">
                    <strong className="d-block">{doc.name}</strong>
                    <small className="text-muted fst-italic">{doc.description || "No description"}</small>
                  </div>
                  <Button variant="outline-danger" size="sm" className="p-1 remove-button-inline flex-shrink-0"
                    onClick={() => handleRemoveDocumentRequirement(index)}
                    aria-label={`Remove ${doc.name}`} disabled={internalIsSaving}> <FaTrash /> </Button>
                </ListGroup.Item>
              ))}
            </ListGroup>
          </Card.Body>
        ...
        <Card className="mb-4 shadow-sm">
          ...
          <Card.Body className="bg-light-subtle">
            ...
            {!docDefLoading && loanData.fields.length === 0 && (
              <p className="text-muted text-center py-3 mb-0">No custom fields added yet.</p>
            )}
            {loanData.fields.map((field, index) => (
              <div className="custom-field-item p-3 mb-3 border rounded bg-white position-relative" key={`custom-field-${index}`}>
                <div className="custom-field-header d-flex justify-content-between align-items-center mb-3 pb-2 border-bottom">
                  ...
                  <Button variant="link" size="sm" className="p-0 text-danger remove-button-inline"
                    onClick={() => handleRemoveCustomField(index)} aria-label={`Remove Custom Field ${index + 1}`}
                    disabled={internalIsSaving} title="Remove Field"> <FaTrash /> </Button>
                </div>
                <Row>
                ...
                <Row className="align-items-end">
                  {renderCustomFieldSelect(index, "type", "Field Type",
                    [
                      { value: "text", label: "Text" }, { value: "textarea", label: "Text Area" },
                      { value: "number", label: "Number" }, { value: "date", label: "Date" },
                      { value: "datetime", label: "Date & Time" }, { value: "time", label: "Time" },
                      { value: "select", label: "Dropdown (Single)" }, { value: "multiselect", label: "Dropdown (Multi)" },
                      { value: "checkbox", label: "Checkbox" }, { value: "image", label: "Image Upload" },
                ...
                {field.type !== "image" && field.type !== "document" && !docDefLoading && ( // Ensure options are loaded
                  <Row>
                    {renderCustomFieldMultiSelect(index, "auto_fill_sources", "Potential Auto-Fill Sources", allAutoFillSourceOptions, { md: 12 })}
                  </Row>
                )}
              </div>
            ))}
            <div className="mt-2 text-end">
              <Button type="button" variant="outline-primary" size="sm" onClick={handleAddCustomField} disabled={internalIsSaving}>
                <FaPlus className="me-1" /> Add Custom Field </Button>
            </div>
          </Card.Body>
        </Card>
