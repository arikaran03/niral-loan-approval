...
const generateDocumentDescription = (docKey) => {
  if (!PREDEFINED_DOCUMENTS[docKey]) return "";
  const fields = PREDEFINED_DOCUMENTS[docKey].fields
    .map((f) => f.label)
    .join(", ");
  return `Expected Fields: ${fields}`;
};
const generateAutoFillOptions = () => {
  const opts = [];
  for (const dk in PREDEFINED_DOCUMENTS) {
    PREDEFINED_DOCUMENTS[dk].fields.forEach((f) =>
      opts.push({
        value: `${dk}.${f.key}`,
        label: `${PREDEFINED_DOCUMENTS[dk].label} - ${f.label}`,
        key: f.key,
      })
    );
  }
...
const LoanFormBuilder = ({
  ...
  const getInitialState = useCallback(
    () => ({
      ...
      required_documents: Array.isArray(initialData?.required_documents)
        ? initialData.required_documents.map((d) => ({ ...d }))
        : [],
  ...
  const [selectedDocumentType, setSelectedDocumentType] = useState(
    Object.keys(PREDEFINED_DOCUMENTS)[0]
  );
  ...
  const validate = (isPublishing = false) => {
    ...
    if (requirePublish && !loanData.description?.trim())
      newErrors.description = "Description is required for publishing.";

    // Financial Details
    if (
      (requirePublish || loanData.min_amount !== "") &&
    ...
    if (
      (requirePublish || loanData.max_amount !== "") &&
    ...
    if (
      (requirePublish || loanData.interest_rate !== "") &&
    ...
    if (
      (requirePublish || loanData.tenure_months !== "") &&
    ...
    // Dates
    if (requirePublish && !loanData.application_start)
      newErrors.application_start =
        "Application start date is required for publishing.";
    if (requirePublish && !loanData.application_end)
    ...
    loanData.fields.forEach((field, index) => {
      ...
      if (currentFieldId && fieldIds.has(currentFieldId.trim())) {
        newErrors[
          `${prefix}.field_id`
        ] = `Field ID "${currentFieldId}" is already used or derived multiple times. IDs must be unique.`;
    ...
    if (isPublishing && loanData.required_documents.length === 0) {
      newErrors["required_documents"] =
        "At least one required document must be specified for publishing.";
    }
  ...
  const handleCustomFieldChange = (index, event) => {
    ...
    } = event.target; // Get name from event.target
    const fieldName = name; // Use name directly
    const isCheckbox = type === "checkbox";
    const isMultiSelect =
      isMultiSelectTarget && fieldName === "auto_fill_sources"; // Check if it's the multi-select target
  ...
  const handleAddDocumentRequirement = () => {
    ...
    if (
      !docConfig ||
      loanData.required_documents.some((doc) => doc.name === docConfig.label)
    )
      ...
      return (
        ...
        <Col md={options.md || 6} className="mb-3">
          ...
          >
            <div>
              {" "}
              {/* Extra div prevents label stretching */}
              <Form.Check
  ...
  const renderCustomFieldMultiSelect = (
    ...
    let filteredOptions;
    if (baseKey) {
      filteredOptions = baseOptionsArray.filter(
          ...
          <Card.Body>
            <Row>
              {renderInput("title", "Loan Title", "text", {
                required: true,
                md: 12,
                placeholder: "Enter a clear title for the loan scheme",
              })}
              {renderTextArea("description", "Description (HTML supported)", {
                ...
                placeholder:
                  "Describe the loan details, benefits, target audience...",
              })}
            </Row>
          </Card.Body>
        </Card>
            ...
            <Row className="g-3 align-items-center mb-3 pb-3 border-bottom">
              <Col xs={12} sm>
                <Form.Select
                  ...
                  disabled={internalIsSaving}
                  aria-label="Select document type to add"
                >
            ...
            <ListGroup variant="flush">
              {loanData.required_documents.length === 0 && (
                <ListGroup.Item className="text-muted text-center py-3">
                  No standard documents added yet.
                </ListGroup.Item>
              )}
