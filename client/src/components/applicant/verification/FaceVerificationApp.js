...
const FACE_VERIFICATION_SERVICE_BASE_URL =
  "https://45vz631c-5000.inc1.devtunnels.ms"; // Your FV Service URL
...
function FaceVerificationApp({ referenceImageId, onVerificationComplete }) {
  ...
  const [isReferenceProcessedByFVBackend, setIsReferenceProcessedByFVBackend] =
    useState(false);
  ...
  const [initialImagePreviewManual, setInitialImagePreviewManual] =
    useState(null);
  ...
  const resetAllTimers = useCallback(() => {
    if (startFramingTimeoutRef.current)
      clearTimeout(startFramingTimeoutRef.current);
  ...
  const stopLiveVerificationProcess = useCallback(
    (message, type = "failed", internalStop = false) => {
      console.log(
        "FV Client: Stopping live verification. Message:",
        message,
        "Type:",
        type
      );
      ...
      if (!internalStop && typeof onVerificationComplete === "function") {
        onVerificationComplete(
          success,
          success ? "" : message || "Verification process ended."
        );
      }
    },
  ...
  useEffect(() => {
    ...
    if ((referenceImageId || initialImage) && !socketRef.current) {
      console.log(
        `FV Client: Attempting to connect to FV Socket.IO server at ${FV_SOCKET_SERVER_URL} using POLLING only.`
      );
      ...
      newSocket.on("connect", () => {
        console.log(
          "FV Client: Socket.IO Connected! SID:",
          newSocket.id,
          "Transport:",
        ...
        if (referenceImageFile && !isReferenceProcessedByFVBackend) {
          // Image fetched via ID is ready
          setStatusMessage(
            "Processing reference image with verification service..."
          );
          uploadReferenceToFVBackend(referenceImageFile, newSocket.id);
        } else if (
          initialImage &&
          !referenceImageId &&
          !isReferenceProcessedByFVBackend
        ) {
          // Manual image is ready
          setStatusMessage("Processing uploaded reference image...");
          uploadReferenceToFVBackend(initialImage, newSocket.id);
        } else if (referenceImageId && !referenceImageFile) {
          // ID provided, but image not fetched yet
          setStatusMessage("Awaiting reference image data...");
        } else if (isReferenceProcessedByFVBackend) {
          // Already processed
          setStatusMessage('Reference ready. Click "Start Live Verification".');
          setVerificationStatus("ready_to_verify");
        } else if (!referenceImageId && !initialImage) {
          // Manual mode, no image selected yet
      ...
      newSocket.on("disconnect", (reason) => {
        ...
        if (
          ...
          verificationTimeoutRef.current ||
          startFramingTimeoutRef.current
        ) {
          stopLiveVerificationProcess(
            "Disconnected from verification service.",
            "failed"
          );
        } else {
      ...
      newSocket.on("connect_error", (err) => {
        ...
        if (typeof onVerificationComplete === "function")
          onVerificationComplete(false, errMsg);
      });
      ...
      newSocket.on("verification_status", (data) => {
        ...
        if (
          data.status === "started" &&
          verificationStatus !== "streaming" &&
          verificationStatus !== "verifying"
        ) {
          // Ensure client is in a state that expects streaming to start
          setVerificationStatus((prevStatus) =>
            prevStatus === "webcam_starting" || prevStatus === "ready_to_verify"
              ? "streaming"
              : prevStatus
          );
        }
      ...
      newSocket.on("verification_result", (data) => {
        ...
        if (isActiveClientProcess) {
          ...
          // Process was already stopped on client, but server might send a late result
          console.log(
            "FV Client: Received 'verification_result' but client process was already stopped/not active:",
            data
          );
          if (data.status === "success" && verificationStatus !== "success") {
            console.warn(
              "FV Client: Server reported SUCCESS after client process was stopped/failed. Overriding to success."
            );
  ...
  useEffect(() => {
    ...
    if (
      ...
      const fetchImage = async () => {
        console.log(
          `FV Client: Fetching reference image with ID: ${referenceImageId}`
        );
        ...
        try {
          ...
          const response = await axiosInstance.get(
            `/api/image/${referenceImageId}`,
            {
              responseType: "blob",
            }
          );
          const file = new File([response.data], "reference_image.jpg", {
          ...
          setStatusMessage(
            "Reference photo fetched. Waiting for verification service connection if not already connected..."
          );
        } catch (error) {
          console.error("FV Client: Error fetching reference image:", error);
          const errMsg = `Failed to fetch reference image: ${
  ...
  useEffect(() => {
    ...
    if (
      ...
      const imageToProcess = referenceImageId
        ? referenceImageFile
        : initialImage;
      if (imageToProcess) {
        console.log(
          "FV Client: Effect4 trigger - Socket connected and reference image ready. Uploading to FV backend."
        );
  ...
  const uploadReferenceToFVBackend = async (fileToProcess, currentSocketId) => {
    if (!fileToProcess || !currentSocketId) {
      console.warn(
        "FV Client: Cannot process reference, file or socketId missing."
      );
    ...
    console.log(
      `FV Client: Uploading reference to FV backend. Socket ID: ${currentSocketId}`
    );
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.message || `FV Backend Error: ${response.status}`
        );
      }
      console.log(
        "FV Client: Reference image processed by FV backend successfully.",
        result
      );
      setIsReferenceProcessedByFVBackend(true);
      setVerificationStatus("ready_to_verify");
      setStatusMessage(
        'Reference image ready. Click "Start Live Verification".'
      );
      if (!referenceImageId) setUploadInitialError(""); // Clear manual upload error on success
    } catch (error) {
      console.error(
        "FV Client: Error processing reference image with FV backend:",
        error
      );
  ...
  const handleManualImageChange = (event) => {
    ...
    if (file) {
      ...
      if (initialImagePreviewManual)
        URL.revokeObjectURL(initialImagePreviewManual);
      ...
      setStatusMessage(
        'Manual image selected. Click "Use This Image" to process.'
      );
    } else {
  ...
  const handleManualImageUpload = () => {
    ...
    if (!socketId) {
      ...
      setUploadInitialError(
        "Not connected to verification service. Please wait."
      );
      // Attempt to connect socket if not already trying
      if (!socketRef.current) {
        // This will trigger Effect 2
        console.log(
          "FV Client: Manual upload triggered, attempting socket connection first."
        );
  ...
  const handleRetryLiveVerification = () => {
    console.log("FV Client: User initiated retry for live verification.");
    // This function re-initiates the live verification process.
    // startLiveVerification() already handles setting the correct statuses and timeouts.
  ...
  useEffect(() => {
    // Manages the frame sending interval
    let intervalId = null;
    if (
      verificationStatus === "streaming" ||
      verificationStatus === "verifying"
      ...
      // Not streaming or verifying
      if (frameIntervalRef.current) {
        clearInterval(frameIntervalRef.current);
        frameIntervalRef.current = null;
        console.log(
          "FV Client: Frame sending interval cleared because status is no longer streaming/verifying."
        );
      }
    }
    return () => {
      if (frameIntervalRef.current) {
        ...
        frameIntervalRef.current = null;
        console.log(
          "FV Client: Frame sending interval cleared on effect cleanup for verificationStatus/sendFrameToFVServer."
        );
      }
    };
  ...
  const startLiveVerification = async () => {
    if (!isReferenceProcessedByFVBackend) {
      ...
      setProcessingReferenceError(
        "Reference image not ready for verification."
      );
      if (typeof onVerificationComplete === "function")
        onVerificationComplete(false, "Reference image not ready.");
      return;
    }
    if (!socketRef.current || !socketRef.current.connected) {
      ...
      if (typeof onVerificationComplete === "function")
        onVerificationComplete(false, "Not connected to verification service.");
      return;
    }
          ...
          try {
            ...
            startFramingTimeoutRef.current = setTimeout(() => {
              if (streamRef.current && streamRef.current.active) {
                console.log(
                  "FV Client: Delay complete. Setting status to STREAMING."
                );
                setVerificationStatus("streaming"); // This will trigger the useEffect for setInterval
              } else {
                stopLiveVerificationProcess(
                  "Webcam became inactive.",
                  "failed"
                );
              }
            }, START_FRAMING_DELAY_MS);

            const totalClientTimeout =
              VERIFICATION_TIMEOUT_MS + START_FRAMING_DELAY_MS;
            ...
            stopWebcam();
            if (typeof onVerificationComplete === "function")
              onVerificationComplete(false, "Could not play video stream.");
          }
        };
  ...
  if (referenceImageId && isFetchingReference) {
    return (
      <div className="text-center my-3 p-3 border rounded bg-light">
        <Spinner animation="border" size="sm" className="me-2" /> Fetching
        reference photo for verification...
      </div>
    );
  }
      ...
      <Card.Body>
        {!referenceImageId && (
          <div className="mb-4 p-3 border rounded">
            ...
            {uploadInitialError && (
              <Alert variant="danger" className="mt-1 py-1 small">
                {uploadInitialError}
              </Alert>
            )}
            ...
            <Button
              ...
              className="w-100"
            >
              {isUploadingInitial && verificationStatus === "processing_ref" ? (
                <>
          ...
          <Row className="mb-3 align-items-center p-2 border rounded bg-light-subtle">
            ...
            <Col>
              <small className="text-muted d-block">
                Reference photo (from Aadhaar) loaded.
              </small>
              {isReferenceProcessedByFVBackend &&
                verificationStatus !== "success" &&
                verificationStatus !== "failed" && (
                  ...
                  >
                    <FaCheckCircle size={12} /> Ready for live check
                  </Badge>
                )}
              {verificationStatus === "processing_ref" && (
                <Badge
        ...
        {(isReferenceProcessedByFVBackend ||
          (!referenceImageId && isReferenceProcessedByFVBackend)) &&
          verificationStatus !== "success" &&
          verificationStatus !== "failed" &&
          verificationStatus !== "error" && (
            <>
              <div
                className="text-center mb-2 webcam-feed-container bg-light p-1"
                style={{
                  minHeight: "180px",
                  display: "flex",
              ...
              >
                ...
                {!streamRef.current && (
                  <p className="text-muted small m-0">
                    Webcam will appear here for live verification.
                  </p>
                )}
              </div>
              {webcamError && (
        ...
        <Alert
          ...
          className="text-center py-2 status-alert small"
        >
          {isLoading && (
            <Spinner
              as="span"
        ...
        {verificationStatus !== "success" && (
          <div className="d-grid gap-2 mt-2">
            {verificationStatus === "failed" ? (
              <Button
                variant="primary"
                onClick={handleRetryLiveVerification}
                size="sm"
              >
              ...
              <Button
                ...
                disabled={
                  ...
                  isLoading ||
                  verificationStatus === "success" ||
                  verificationStatus === "webcam_starting" ||
                  verificationStatus === "streaming" ||
                  verificationStatus === "verifying"
                }
                size="sm"
            ...
            {(isLoading ||
              verificationStatus === "failed" ||
              verificationStatus === "error" ||
              verificationStatus === "ready_to_verify") &&
              verificationStatus !== "idle" &&
              verificationStatus !== "success" && (
                <Button
                  variant="outline-secondary"
                  size="xs"
                  onClick={handleStopProcessByUser}
                >
                  Cancel
                </Button>
              )}
